...
namespace :deploy do
  task :start, :roles => :app, :except => { :no_release => true } do
    run "cd #{current_path} && #{current_path}/tmp/ #{passenger_cmd} start -e #{rails_env} -p #{passenger_port} -d"
  end

  task :stop, :roles => :app, :except => { :no_release => true } do
    run "cd #{current_path} && #{current_path}/tmp/ #{passenger_cmd} stop -p #{passenger_port}"
  end
  ...
  task :restart, :roles => :app, :except => { :no_release => true } do
    run <<-CMD
      ...
      then
        cd #{current_path} && #{current_path}/tmp/ #{passenger_cmd} stop -p #{passenger_port};
      fi
    CMD
